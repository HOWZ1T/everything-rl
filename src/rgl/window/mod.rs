...
impl<'a, T, S> Window<'a, T, S>
where
    T: FnMut(&mut glfw::Window, WindowEvent, &mut S)
{
    pub fn new(glfw: &mut Glfw, width: u32, height: u32, title: &'a str, event_handler: T) -> Result<Window<'a, T, S>, WindowError> {
    ...
    pub fn poll_events(&mut self, glfw: &mut Glfw, state: &mut S) {
        glfw.poll_events();
